...
def privateKeyToWif(key_hex):
    return utilis.base58CheckEncode(0x80, key_hex.decode('hex'))


def wifToPrivateKey(s):
    b = utilis.base58CheckDecode(s)
    return b.encode('hex')
...
def pubKeyToAddr(s):
    ...
    return utilis.base58CheckEncode(0, ripemd160.digest())


def addrHashToScriptPubKey(b58str):
    assert (len(b58str) == 34)
    # 76     A9      14 (20 bytes)                                 88             AC
    return '76a914' + utilis.base58CheckDecode(b58str).encode('hex') + '88ac'
...
class TestKey(unittest.TestCase):
    ...
    def test_pairs1(self):
        ...
        wallet_key = utilis.base256encode(utilis.base58decode(wallet_private)).encode('hex')
        ...
        wallet_key = utilis.base58CheckDecode(bitcoin_qt).encode('hex')
        self.assertEqual(keyToAddr(wallet_key), wallet_addr)
    ...
    def test_pairs3(self):
        ...
        self.assertEqual(keyToAddr(utilis.base58CheckDecode(wallet_private).encode('hex')), wallet_addr)
